...
The tests directory contains a single host file that uses php to make it possible to run all of the tests and examples from the single host file.  This page takes a few parameters to indicate which test or example you wish to run.  The only required parameter is 'test':
...
Currently, there's no link list or convienent way to see which tests and examples are available.  The most up to date list is scrapable from the targets in the build.felix.xml file.

The progress of cajoling the library is currently documented in the test_results.txt file.
...
* Create a linked directory of cajoled tests and examples
