...
The tests directory contains an index.php file that includes a link list to all cajoled tests and examples (working or not).  All tests and examples use a single host.php file to make it possible to run all of the tests and examples from the single host file.  This page takes a few parameters to indicate which test or example you wish to run.  The only required parameter is 'test':
